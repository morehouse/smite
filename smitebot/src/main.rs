...
//!
//! Currently exposes `smitebot doctor`, which validates host prerequisites
//! required for running Smite fuzzing campaigns.
...
/// Dispatches the parsed top-level CLI command.
fn run(cli: Cli) -> bool {
    match cli.command {
...
fn run_doctor(args: &DoctorArgs) -> bool {
...
fn expand_tilde_with_home(path: &Path, home: &OsStr) -> PathBuf {
