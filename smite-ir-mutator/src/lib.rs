...
impl MutatorState {
    ...
    ///
    /// If `splice` is `Some`, `SpliceMutator` is added to the pool of available
    /// mutators, allowing the `splice` program to be randomly inserted into the
    /// target `program` during stack execution.
    ///
...
mod tests {
    ...
    fn fuzz_with_splice_utilizes_splice_mutator() {
        ...
        // rolls `SpliceMutator` to guarantee the FFI correctly handed the
        // payload to `mutate_stacked()`.
        for _ in 0..100 {
        ...
        assert!(
            splice_used,
            "SpliceMutator was never chosen despite a valid add_buf"
        );
    }
